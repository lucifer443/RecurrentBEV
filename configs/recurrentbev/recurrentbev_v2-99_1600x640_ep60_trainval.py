...
pretrain = 'checkpoints/dd3d_det_final.pth'
...
model = dict(
    ...
    img_neck=dict(in_channels=[256, 512, 768, 1024]),
    aux_2d_branch=dict(
        neck=dict(in_channels=[256, 512, 768, 1024])),
...
_base_.train_multiview_transforms[0].update(
    dict(ratio_range=(0.94, 1.11))
)
_base_.train_multiview_transforms[1].update(
    dict(crop_size=(1600, 640))
)
...
train_dataloader = dict(
    batch_size=4,
    dataset=dict(
        ann_file='data/nuscenes/annotations/nuscenes_temporal_infos_trainval.pkl',
...
default_hooks = dict(checkpoint=dict(type='CheckpointHook', interval=5, max_keep_ckpts=6),)
